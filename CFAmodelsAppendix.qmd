...
The Merck data came with a password. Please contact the authors for access to the data. NOTE: The data are not included in the GitHub repository.
...
knitr::kable(df.varnames, row.names = FALSE, caption = "Table 1.  Workplace Curiosity Scale Items and Factors", align = "c") %>% 
...
MODEL 1: **Current Model:** We had a four factor, four item/factor model that we previously found in our original analyses.
...
-   **Configural or No Constraints Model:** We tested the configural model to see if the factor structure was the same across the two groups.  
-   **Weak Constraints Model:** We tested the weak constraints model to see if the factor loadings were the same across the two groups.
-   **Strong Constraints Model:** We tested the strong constraints model to see if the factor loadings and intercepts were the same across the two groups.
...
## Current vs. New Model (Locale; Fit Statistics)

The table below shows us the fit statistics for the current and new models across the three types of models (configural, weak, and strong constraints) by locale.  Our results here indicate several things.  First, the new model consistently outperforms the current measurement model.  Every model specifying only 3 indicators for each of the four factors fit well and fit better than the more complicated model with 4 indicators per factor.  Second, the weakly constrained, new model performed best across Locale.  This second point we shall return to after we see the results from the other multisample analyses.  
...
#| include: true
#| cache: true
Constraints <- c("Configural", "Configural", "Weak", "Weak", "Strong", "Strong")
Model <- c("Current", "New", "Current", "New", "Current", "New")

tabLocale <- round(rbind(fitMeasures(fit.config.cur, c("cfi","rmsea","srmr")),
      fitMeasures(fit.config.new, c("cfi","rmsea","srmr")),
      fitMeasures(fit.weak.cur, c("cfi","rmsea","srmr")),
      fitMeasures(fit.weak.new, c("cfi","rmsea","srmr")),
      fitMeasures(fit.strong.cur, c("cfi","rmsea","srmr")),
      fitMeasures(fit.strong.new, c("cfi","rmsea","srmr"))),3)
colnames(tabLocale) <- c("CFI", "RMSEA", "SRMR")
...
tabLoc %>%
  arrange(Model) %>%
  select(Model, Constraints, CFI, RMSEA, SRMR) %>%
  gt() %>%
  tab_header(
    title = "Fit Statistics by Locale"
  ) %>%
  fmt_number(
    columns = c(CFI, RMSEA, SRMR),
    decimals = 3
  )
...
These model comparisons must be conducted **between** constraints but **within** models.  Thus, the results for our _current_ model across the three types of constraints are as follows:

```{r CompConstraintCurrentByLocale}
#| echo: true
#| messages: false
#| errors: false
#| warnings: false
...
kable(lavTestLRT(fit.config.new, fit.weak.new, fit.strong.new))
```
The new model fits better than the current model.  


## CFA Summary by Locale

Weakly constrained models fit best for both the current (old) and new models. Given the relative fit statistics between models, we hold that the new model is more defensible. Overall, the fit by CFI (0.978 vs. 0.951) and RMSEA (0.043 vs. 0.055) for the new vs. old, respectively. These results suggest that the new model is a better fit to the data and, in fact, provide clearer evidence of model fit - at least weakly by constrained factor loadings - for a measurement model that generalizes beyond single samples.
...
## Current vs. New Model (Age Group; Fit Statistics)
...
tabAge <- round(rbind(fitMeasures(fit.conf.age.cur, c("cfi","rmsea","srmr")),
      fitMeasures(fit.conf.age.new, c("cfi","rmsea","srmr")),
      fitMeasures(fit.weak.age.cur, c("cfi","rmsea","srmr")),
      fitMeasures(fit.weak.age.new, c("cfi","rmsea","srmr")),
      fitMeasures(fit.strong.age.cur, c("cfi","rmsea","srmr")),
      fitMeasures(fit.strong.age.new, c("cfi","rmsea","srmr"))),3)
colnames(tabAge) <- c("CFI", "RMSEA", "SRMR")
...
names(byAge.cfa.cur) <- c("configural","weak","strong")
names(byAge.cfa.new) <- c("configural","weak","strong")
kable(byAge.cfa.cur, 
      caption = "Fit Statistics by Age Group (Current Model)")
kable(byAge.cfa.new,
      caption = "Fit Statistics by Age Group (New Model)")
```


### Average Standardized Factor Loadings by Age Group
...
tidy(fit.strong.age.new) %>%
  ...
  summarize(BetaMean = round(mean(std.all),2), 
            BetaSD = round(sd(std.all),2), 
            SEMean = round(mean(std.error),2)) %>%
  kable(caption = "Mean Standardized Factor Loadings across Age Groups (New Model)")
...
## Current vs. New Model (Gender; Fit Statistics)
...
tabGender <- round(rbind(fitMeasures(fit.conf.sex.cur, c("cfi","rmsea","srmr")),
      fitMeasures(fit.conf.sex.new, c("cfi","rmsea","srmr")),
      fitMeasures(fit.weak.sex.cur, c("cfi","rmsea","srmr")),
      fitMeasures(fit.weak.sex.new, c("cfi","rmsea","srmr")),
      fitMeasures(fit.strong.sex.cur, c("cfi","rmsea","srmr")),
      fitMeasures(fit.strong.sex.new, c("cfi","rmsea","srmr"))),3)
colnames(tabGender) <- c("CFI", "RMSEA", "SRMR")
tabGender <- data.frame(Model, Constraints, tabGender)
knitr::kable(tabGender, caption = "Fit Statistics by Age Group")
```

## Compare Constraints (Gender)
...
names(bySex.cfa.cur) <- c("configural","weak","strong")
names(bySex.cfa.new) <- c("configural","weak","strong")
kable(bySex.cfa.cur)
kable(bySex.cfa.new)
```
...
To supplement and potentially test for the robustness of the CFA results, we ran two EFA models - the current and new model items.  The results are as follows:

## EFA Model - ALL ITEMS (Current Model)
...
ggcorrplot(corItems, type = "lower", 
           method="circle", 
           lab = T, 
           title = "",
           show.legend = F,
...
Compared to the correlations with  and Q3, Q4 and Q6, Q7 and Q8, and Q10 and Q11.  These items are likely measuring the same construct.  We will test this hypothesis in the next section.
...
tbl_merge(tbls = list(lmDS.loc, lmJE.loc, lmST.loc, lmOP.loc),
  ...
  modify_footnote(label = "Note: Factor scores are computed using the CFA model.  US is the reference group.")
...
tbl_merge(tbls = list(lmDS.age, lmJE.age, lmST.age, lmOP.age),
          tab_spanner = c("DS","JE","ST","OP")) %>%
...
tbl_merge(tbls = list(lmDS.sex, lmJE.sex, lmST.sex, lmOP.sex),
          tab_spanner = c("DS","JE","ST","OP")) %>%
...
The results of the CFA models are summarized in the following tables. The first table provides the fit statistics for New and Current models across the entire sample (later) and the MS models. The second table provides the results of the likelihood ratio tests for the constraints.
...
# Fit statistics for New and Current models across the entire sample and the MS models
...
tabStage2 <- rbind(tabLoc, tabAge, tabGender) %>%
...
#tabStage2 %>%
#  kable(caption = "Fit Statistics by Facet and Constraint", digits=2)

#tabStage2 %>%
#  gt()

tabStage2 %>%
  apa("Fit Statistics By Facet and Constraint") # uses gt() package - function from above

#library(semTools)
#measurementInvariance(model=New.model, 
#                      data = df.fin, 
#                     group = "Locale")
