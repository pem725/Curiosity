...
tab1Stage3a <- rbind(tabLoc, tabAge, tabGender) %>%
...
ageMC <- tbl_merge(tbls = list(lmDS.age, lmJE.age, lmST.age, lmOP.age),
          tab_spanner = c("DS","JE","ST","OP")) %>%
  modify_header(label = "Factor Scores by Age") %>%
  modify_footnote(label = "Note: Factor scores are computed using the CFA model.  18-24 is the reference group with effects coding.")
...
sexMC <- tbl_merge(tbls = list(lmDS.sex, lmJE.sex, lmST.sex, lmOP.sex),
          tab_spanner = c("DS","JE","ST","OP")) %>%
  modify_header(label = "Factor Scores by Sex") %>%
...
df.fin %>%
  ...
  mutate(Factor = recode(Factor, ds3sum = "DS",je3sum = "JE",st3sum = "ST",op3sum = "OP")) %>%
  gt::gt() %>%
  gt::gtsave("locDiff.png")


df.fin %>%
  select(Age, ds3sum:op3sum) %>%
  pivot_longer(cols = ds3sum:op3sum, names_to = "Factor", values_to = "Sum Scores") %>%
  group_by(Factor, Age) %>%
  summarise(Mean = mean(`Sum Scores`, na.rm = T), SD = sd(`Sum Scores`, na.rm = T), .groups = "drop") %>%
  mutate(across(where(is.numeric), round, 2)) %>%
...
#Make the tables for the factors so that readers can read across the table to do the comparisons by factor.
