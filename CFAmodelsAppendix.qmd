...
kable(lavTestLRT(fit.config.new, 
                 ...
                 fit.strong.new))
```
The new model fits better than the previous model.  The weakly constrained model fits best for both the current (old) and new models.  Given the relative fit statistics between models, we hold that the new model is more defensible.  Overall, the fit by CFI (0.978 vs. 0.951) and RMSEA (0.043 vs. 0.055) for the new vs. old, respectively.  These results suggest that the new model is a better fit to the data and, in fact, provide clearer evidence of model fit - at least weakly by constrained factor loadings - for a measurement model that generalizes beyond single samples.


## CFA Summary by Locale

The Metric invariance models fit best for both the current (old) and new models. Given the relative fit statistics between models, we hold that the new model is more defensible. Overall, the fit by CFI (0.978 vs. 0.951) and RMSEA (0.043 vs. 0.055) for the new vs. old, respectively. These results suggest that the new model is a better fit to the data and, in fact, provide clearer evidence of model fit - at least weakly by constrained factor loadings - for a measurement model that generalizes beyond single samples.
...
## Current vs. New Model (Age; Fit Statistics)
...
allModelsFit <- as.data.frame(round(
  cbind("Previous: Lower"=fitmeasures(fit.Cur.ALL),
        "Previous: Higher"=fitmeasures(fit.HOF.cur),
        "New: Lower"=fitmeasures(fit.New.ALL),
        "New: Higher"=fitmeasures(fit.HOF.new)
        ),2))
kable(allModelsFit[c("cfi","tli","rmsea","srmr"),])
```
...
tabStage3a %>%
  apa("Fit Statistics By Facet and Constraint") %>%
