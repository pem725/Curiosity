...
if (!file.exists("Merck Curiosity Survey_Data_NO_PWD.csv")) {
  # read a xlsx file with a password and save to a data frame
  ...
  write.csv(as.data.frame(t(df.varnames)), file = "Merck Curiosity Survey_Data_NO_PWD_varnames.csv", row.names = FALSE)
}
...
df.fin <- read.csv("Merck Curiosity Survey_Data_NO_PWD.csv", header = TRUE)

# read the variable names
df.varnames <- read.csv("Merck Curiosity Survey_Data_NO_PWD_varnames.csv", header = TRUE)
...
```{r}
# run a system command in R and export the results back to R as a variable
md5sum <- system("md5sum Merck Curiosity Survey_Data_PWD.xlsx", intern = TRUE)
# print the md5sum
print(md5sum)
```
